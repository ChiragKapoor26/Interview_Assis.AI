...
// // useFacialAnalysis.ts — around line 92 where FaceLandmarker is created

// const initFaceLandmarker = async () => {
//   // 👇 Save and clear define to avoid AMD conflict with Monaco
//   const originalDefine = (window as any).define;
//   const originalDefineAmd = (window as any).define?.amd;
...
//   } finally {
//     // 👇 Restore AMD loader for Monaco
//     if (originalDefine) {
//       (window as any).define = originalDefine;
//       if (originalDefineAmd) {
//         (window as any).define.amd = originalDefineAmd;
//       }
//     }
//   }
// }
...
export function useFacialAnalysis(
  ...
  const runDetection = useCallback(() => {
    ...
    try {
      ...
      // Silently skip frame errors to avoid spam
    }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        // 👇 STEP 1: Resolve Fileset (no AMD conflict here)
        const vision = await FilesetResolver.forVisionTasks(WASM_URL)
        if (cancelled) return

        // 👇 STEP 2: Temporarily disable AMD `define` to avoid Monaco conflict
        const originalDefine = (window as any).define
        const originalDefineAmd = (window as any).define?.amd
        ...
        try {
          ...
          // 👇 STEP 3: Create FaceLandmarker (now safe from AMD conflict)
          landmarkerRef.current = await FaceLandmarker.createFromOptions(vision, {
            baseOptions: { 
          ...
          // 👇 STEP 4: Restore AMD loader for Monaco Editor
          if (originalDefine) {
            (window as any).define = originalDefine
            if (originalDefineAmd) {
              (window as any).define.amd = originalDefineAmd

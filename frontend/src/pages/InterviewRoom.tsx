      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-2 pt-4 pointer-events-none">
        <div className="space-y-1.5">
          <MetricBar
            ...
            label="Eye Contact"
            value={metrics.faceDetected ? metrics.eyeContact : 0}
            color="text-blue-400"
          />
          <MetricBar
            icon={<Smile className="w-3 h-3" />}
            label="Expression"
            value={metrics.faceDetected ? metrics.smileRatio : 0}
            color="text-yellow-400"
          />
          <MetricBar
            icon={<Zap className="w-3 h-3" />}
            label="Confidence"
            value={metrics.faceDetected ? metrics.confidence : 0}
...
export default function InterviewRoom({ user }: { user: any }) {
  ...
  // Log a snapshot every 5 s while interview is live
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${wsUrl}/ws/interview/${interviewId}`)
    wsRef.current = ws
    ...
    ws.onmessage = (event) => {
      ...
      if (msg.type === 'audio' && msg.data) {
        const blob = new Blob([Uint8Array.from(atob(msg.data), c => c.charCodeAt(0))], { type: 'audio/mpeg' })
        const url = URL.createObjectURL(blob)
        if (audioRef.current) { audioRef.current.src = url; audioRef.current.play() }
        if (audioRef.current) audioRef.current.onended = () => setIsAgentSpeaking(false)
      }
      if (msg.type === 'interview_complete') setInterviewDone(true)
    }

    return () => { ws.close(); stopRecognition() }
  }, [interviewId])
  ...
  const startRecognition = useCallback(() => {
    ...
    recog.onresult = (e: any) => {
      ...
      if (transcript && wsRef.current?.readyState === 1) {
  ...
  const stopRecognition = () => { recognitionRef.current?.stop(); setIsListening(false) }

  // ── End interview ─────────────────────────────────────────
  const summarizeFacialMetrics = () => {
    const log = facialMetricsLog.current
    if (!log.length) return null
    const avg = (key: string) => log.reduce((s, m) => s + (m[key] || 0), 0) / log.length
    return {
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 overflow-hidden">
        ...
        <div className="lg:col-span-4 border-r bg-muted/20 flex flex-col p-4 gap-4 overflow-y-auto">
          ...
          <Card className="bg-card">
            <CardContent className="p-5">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 rounded-full bg-primary flex items-center justify-center font-bold text-lg shadow-[0_0_15px_rgba(var(--primary),0.3)]">A</div>
              ...
              <div className="min-h-[60px] text-sm text-muted-foreground leading-relaxed">
                {agentText || 'Connecting to Alex...'}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="flex-1 flex flex-col min-h-0 bg-card">
            ...
            <CardContent className="flex-1 overflow-y-auto p-4 space-y-4" ref={transcriptScrollRef}>
              {messages.map((m, i) => (
              ...
              {messages.length === 0 && (
                <div className="text-center text-sm text-muted-foreground py-8">Conversation will appear here...</div>
              )}
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col justify-center">
              <Button
                ...
                disabled={isAgentSpeaking || interviewDone}
              >
      ...
      {interviewDone && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <Card className="max-w-md w-full shadow-2xl border-primary/20 text-center p-6">
            ...
            {/* Show final facial score if available */}
            ...
            <p className="text-muted-foreground mb-8">
              Great job! Alex is generating your personalized feedback report. Head to your dashboard to see how you did.
            </p>
